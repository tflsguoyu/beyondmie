...
function polar_plot1(p, c, logscale)
    [N,M] = size(p);
    assert(M==181);
    assert(size(c,1)==N);
    assert(size(c,2)==3);
...
%     annotation('textbox',[.51 .28 .7 .3],'EdgeColor','none','String','Forward','FontSize',15,'Linewidth',2)
    x = [0.65 0.9];   % adjust length and location of arrow 
    y = [0.23 0.23];
    annotation('textarrow',x,y,'FontSize',13,'Linewidth',2)
    annotation('textbox',[.68 .0 .7 .3],'EdgeColor','none','String','Forward','FontSize',15,'Linewidth',2)
    ...
    % plot phase function
    i=1;
    fill(polar(i,:,1), polar(i,:,2), c(i,:), 'FaceAlpha', 0.0, 'LineWidth', 3, 'EdgeColor', c(i,:), 'EdgeAlpha', 0.8)
    i=2;
    fill(polar(i,:,1), polar(i,:,2), c(i,:), 'FaceAlpha', 0.5, 'LineWidth', 1, 'EdgeColor', c(i,:), 'EdgeAlpha', 0.0)

%     for i  = 1:N
%         fill(polar(i,:,1), polar(i,:,2), c(i,:), 'FaceAlpha', 0.5, 'LineWidth', 1, 'EdgeColor', c(i,:), 'EdgeAlpha', 0.5)
%     end
